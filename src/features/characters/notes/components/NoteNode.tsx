...
// A sticky note that lives on the canvas as a React Flow node.
...
export function NoteNode({ id, data, selected }: NodeProps<NoteNode>) {
  ...
  return (
    <div
      ...
      className={cn(
        "rounded-lg shadow-lg p-3 relative",
    ...
    >
      ...
      >
        {NOTE_COLORS.map((c) => (
          <button
            key={c.bg}
            onClick={() => updateNodeData(id, { color: c.bg })}
            className={cn(
              "w-3.5 h-3.5 rounded-full border transition-transform hover:scale-110",
              data.color === c.bg ? "border-gray-600 scale-110" : "border-transparent"
            )}
